...
import {
    StyleSheet, View, Text, ScrollView, Picker, TouchableNativeFeedback, TouchableOpacity, Alert, BackHandler, TextInput, DeviceEventEmitter 
} from 'react-native';
...
class DetailsRelation extends Component {
    static navigationOptions = ({ navigation }) => {
        const { params } = navigation.state;
        let title = `Solicitud (${params.Relacion})`;
    ...
    validateRequiredFields = (solicitud, hasPrioridadChanged) => {
        ...
        if (solicitud.TipoPrestamoCategoriaId) {
            ...
            if (solicitud.TipoPrestamoCategoriaId === "2") {
                ...
                if (!solicitud.DescripcionOtrosIngresos) {
                    validationMessage += "(*)Descripción Ingresos\n";
                    valid = false;
                }
     ...
     * @returns {Promise<void>}
     */
    async fetchSolicitud(solicitudId, TipoPrestamoCategoriaId, IsEnabledPriority, params) {
        try {
            this.setLoadingState(true);
                                ...
                                this.setState((prevState) => {
                                    return {
                                        ...
                                        solicitud: {
                                            ...
                                            SolicitudId: params.IsNewRelacionado ? '' : data.SolicitudId,
                                            Cedula: params.Cedula,
                                            NombreCompleto: data.NombreCompleto,
                                            Salario: params.IsNewRelacionado ? '' : data.Salario,
                                            OtrosIngresos: params.IsNewRelacionado ? '' : data.OtrosIngresos,
                                            DescripcionOtrosIngresos: params.IsNewRelacionado ? '' : data.DescripcionOtrosIngresos,
                                            TipoEmpleoId: params.IsNewRelacionado ? '' : data.TipoEmpleoId,
                                            TiempoEmpleandoId: params.IsNewRelacionado ? '' : data.TiempoEmpleadoId,
                                            LugarDeTrabajo: params.IsNewRelacionado ? '' : data.LugarDeTrabajo,
                                            TelefonoCasa: params.IsNewRelacionado ? '' : data.TelefonoCasa,
                                            TelefonoCelular: params.IsNewRelacionado ? '' : data.TelefonoCelular,
                                            Email: params.IsNewRelacionado ? '' : data.Email,
                                            NombreEmpresa: params.IsNewRelacionado ? '' : data.NombreEmpresa,
                                            IsSolicitudRelacionado: true,
                                            IsSolicitudGarante: params.IsSolicitudGarante,
                                            IsSolicitudCoDeudor: params.IsSolicitudCoDeudor,
                                            IsSolicitudFiador: params.IsSolicitudFiador,
                                            IdSolicitudDeudor: params.IdSolicitudDeudor,
                                            IsNewSolicitud: params.IsNewSolicitud,
    ...
    async componentDidMount() {
        ...
        if (params) {
            await this.fetchSolicitud(params.SolicitudId, params.TipoPrestamoCategoriaId, params.IsEnabledPriority, params);
        }
    }

    _renderForm = (solicitud, isVehiculo) => {
        return (
            <View style={{ flex: 1 }}>
                <ScrollView keyboardShouldPersistTaps={'always'}>
                    ...
                    <View>
                        <View style={{ flexDirection: 'row' }}>
                            ...
                            {(this.state.isNewSolicitud && this.state.IsNewRelacionado) && this._renderWorkflowEstadoView()}
                        </View>
                        <View>
                            <View style={{ flexDirection: 'row' }}>
                                <View style={{ flex: 1 }}>
                                ...
                                {(this.state.isNewSolicitud && this.state.IsNewRelacionado) && <View style={{ flex: 1 }}>
                            ...
                            {(this.state.isNewSolicitud && this.state.IsNewRelacionado) && <View>
                        ...
                        <View>
                            <View>
                                ...
                                <View>
                                    ...
                                    <TouchableOpacity onPress={this.onOpenSelectDealer} enabled={false}>
                                        <FormInput
    ...
    render() {
        ...
        return (
            <View style={styles.container}>
                {isLoading
                    ? (<LoadingIndicator />)
                    : this._renderForm(solicitud, isVehiculo)}
